...
async function dispatchGitHubWorkflow(
  ...
  if (!token) {
    return {
      ok: false,
      error:
        "Production workers run via GitHub Actions. Set GITHUB_AGENT_TOKEN on Vercel (classic PAT with repo + workflow scopes). Also add DATABASE_URL and GOOGLE_PLACES_API_KEY as GitHub repo secrets.",
    };
  }
  if (token.length < 40) {
    return {
      ok: false,
      error:
        "GITHUB_AGENT_TOKEN on Vercel looks wrong (too short). Paste the full GitHub token, then redeploy.",
    };
  }
  ...
  if (res.status === 403) {
    return {
      ok: false,
      error:
        "GitHub token cannot trigger Actions (403). Use a classic PAT with repo + workflow scopes, update GITHUB_AGENT_TOKEN on Vercel, and redeploy.",
    };
  }
  if (res.status === 401) {
    return {
...
function spawnPythonDetached(options: {
  ...
  const { scriptPath, args, logPath, env, onError } = options;
...
export async function dispatchAnalyzerRun(params: {
  ...
  return spawnPythonDetached({
    scriptPath: path.join(process.cwd(), "agents/analyzer/main.py"),
    args: [params.runId, params.limit],
    logPath: path.join(process.cwd(), "agents/analyzer/worker.log"),
    env: {
      ...process.env,
