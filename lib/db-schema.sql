...
create table leads (
  ...
  lead_type text not null default 'location' check (lead_type in ('location', 'franchise_group')),
  franchise_brand text,
  franchise_location_count int not null default 0,
...
-- Site issue screenshots for pitch emails (mistakes on current website)
